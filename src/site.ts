...
<p><strong>The seed, in full.</strong> The hash of the block after your commit does not exist when your commit is mined, so nobody, you included, can pick a commit block for a known result. The reveal is allowed only from two blocks after the commit, when that hash exists, and only while the chain still serves it, 256 blocks; past that anyone may call <code>renew</code>, which moves the commit to the current block and starts the two-block wait again, so an existing commitment can be recovered without using a missing hash. Always finish your pending roll before starting another, even after midnight UTC. A direct new commit can overwrite the previous one and permanently consume a supply reservation; this site checks for a pending roll before asking you to send. The token number is not in the seed, so ordinary traits do not depend on reveal order. One-of-one allocation does depend on reveal order, because earlier reveals change the remaining pool and supply. The first contract of this collection (two faces) had weaker rules; the repository documents them.</p>
...
seed = keccak(blockhash(commitBlock + 1), wallet, pins, commitBlock) as u64
