...
<p><strong>The roll.</strong> Each wallet may roll once per UTC day while supply remains, in two steps. <code>commit</code> spends your day, fixes your pins and pays the pin fee. From the next block on, <code>reveal</code> mixes the hash of the block after your commit with your address, your pins, the commit block and the token number into a 64-bit seed, and the seed decides everything: seven layers, five colours, and whether this roll takes a one of one. Anyone may reveal for anyone. This site's keeper does it for you, so you sign once; a commit nobody reveals waits, and you can reveal it from your own wallet. A roll without pins has no mint fee; you pay the network gas of one transaction.</p>
...
<p><strong>The seed, in full.</strong> The reveal reads the hash of the block after the commit. A reveal mined in that very block, or more than 256 blocks later, reads that hash as zero, and the token number in the seed depends on the order of reveals. Those are properties of the deployed contract, documented in the repository; the keeper reveals from the block after that hash exists. The site does not claim more than the contract gives.</p>
