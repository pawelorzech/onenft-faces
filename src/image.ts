...
export function cardSvg(title: string, sub: string, t: Traits): string {
  ...
  const p = groundOf(t);
  const tl = fitLine(attributesOf(t).slice(0, 4).map((a) => a.value).join(", "), "Newsreader", 28, 400);
...
<text x="654" y="290" font-family="Syne" font-weight="800" font-size="${fitSize(title, "Syne", 150, 800)}" fill="${p.fg}">${esc(title)}</text>
<text x="662" y="360" font-family="Newsreader" font-size="34" fill="${p.fg}">${esc(rarityOf(t))}</text>
<text x="662" y="412" font-family="Newsreader" font-size="${tl.size}" fill="${p.muted}">${esc(tl.text)}</text>
<text x="662" y="560" font-family="Syne" font-weight="800" font-size="${fitSize("faces.onenft.click", "Syne", 44, 800)}" fill="${p.fg}">faces.onenft.click</text>
</svg>`;
}
...
export function cardPng(key: string, title: string, sub: string, t: Traits, keep: boolean): Uint8Array {
  ...
  if (hit) return hit;
  const png = new Resvg(cardSvg(title, sub, t), { fitTo: { mode: "width", value: 1200 }, font: FONT_OPTS }).render().asPng();
