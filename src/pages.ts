...
export function rarityPage(chain: ChainState | null, epoch: number): string {
  ...
  const colours = [["Hair colour", HAIRS, "pinnable"], ["Top colour", TOPCOLORS, "pinnable"], ["Ground", GROUNDS, "pinnable"], ["Accent", ACCENTS, "pinnable"]].map(([name, list, how]) => `<div><h3 class="syne">${name}, ${(list as any[]).length}, even odds, ${how}</h3><p class="small">${(list as { name: string; main: string }[]).map((c) => `<i style="display:inline-block;width:18px;height:18px;background:${c.main};vertical-align:middle;margin-right:4px;box-shadow:0 0 0 1px var(--line)"></i>${esc(c.name)}`).join(" &nbsp; ")}</p></div>`).join("");
