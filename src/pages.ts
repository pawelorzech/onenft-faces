...
<div><h1 class="syne">${nameHeading(rawName)}</h1><p class="lead" style="margin-top:8px">${mine.length ? `${mine.length} ${plural(mine.length, "face", "faces")}${isAuthor(chain, who) ? ", the treasury" : ""}.` : "No faces yet."}${handle.toLowerCase() !== who.toLowerCase() ? ` <span class="small">${shortAddr(who)}</span>` : ""}</p></div>
${factList}
${whoBlock(chain, status)}
...
${rows.length ? downloadScript() : ""}`;
  return layout(`${rawName} | ${SITE}`, p, body, "/today.png", `/${handle}`, `${mine.length} ${plural(mine.length, "face", "faces")} of ${SITE} held by ${rawName}.`, false);
}
