...
R=$(operator_json_address "$D" FaceRenderer)
for seed in 1 77 17; do
  cast call "$R" "svg(uint64,uint128,uint8)(string)" "$seed" 340282366920938463463374607431768211455 255 --rpc-url "$RPC" > "$CHECK_SVG"
  if bun -e 'import { face } from "./src/faces.ts"; let s=(await Bun.file(process.argv[process.argv.length-1]).text()).trim(); if(s.startsWith("\"")) s=JSON.parse(s); process.exit(s===face(BigInt(process.argv[1])).svg?0:1)' "$seed" "$CHECK_SVG"; then echo "seed $seed: svg == TypeScript"; else echo "seed $seed: MISMATCH"; ok=0; fi
done
