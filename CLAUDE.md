...
- Every wallet may `roll` once per UTC day, free (gas only). The contract mixes `prevrandao`, the wallet, the token id and the block into a 64-bit seed; the seed decides seven layers and five colours. Supply stops at 10,000. The author's wallet gets one free roll a day too (`rollForTreasury`, anyone may call; the site loop does).
- A roll may pin up to three of five things: background, top, eyes, hair or hat, skin tone. Pins go to common or uncommon items only (skins: the human tones). One pin 0.0005 ETH, two 0.0015, three 0.004, forwarded to the author on each roll. Rare and legendary items come only from luck.
- A roll without pins may take a one of one from the pool with odds `poolLeft / tokensLeft`, so on average the pool empties with the supply. Each 1/1 exists once.
...
- **Pins are a uint64**, one byte per key high byte first: background, top, eyes, hair, skin, three spare bytes; 0xff is no pin. `PIN_KEYS` in `src/faces.ts` is the order; the renderer and the site follow it.
