...
One face a day per wallet, rolled on chain. A collection of onenft.click, built from `~/Programowanie/onenft-chainrun` on 2026-09-05. Live at https://faces.onenft.click, contract `0x7C745F4eA367A7A3CD596219A4E428F2eA9A8C4c` on Base mainnet (second contract, 2026-09-06; the first, `0x37747e1c6221848807B2fA060dbf4Be798361752`, holds two faces and is retired) (see `docs/DEPLOYMENTS.md`). Operational identifiers that should not be public live in `CLAUDE.local.md` (gitignored).

## What this is

- Every wallet may roll once per UTC day, free (gas only), as **commit then reveal**: `commit(pins)` spends the day and the fee, `reveal(wallet)` from two blocks later (and within 256) mixes `blockhash(commitBlock + 1)` with the wallet, pins and commit block into a 64-bit seed; past the window anyone may `renew` the commit. Second contract (2026-09-06); the first had weaker rules, see `docs/REVEAL_WINDOW.md`. Anyone may reveal for anyone; the site's keeper (`src/autoclaim.ts`, deployer key) does, so the roller signs once. Supply stops at 10,000 counting pending commits. The author's wallet gets one free roll a day too (`commitForTreasury`, anyone may call; the keeper does).
